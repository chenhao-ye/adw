...
class Target:
    ...
    def gen_code(self, f, target_dir: str) -> None:
        ...
        f.write("# Generated by ADW. DO NOT MODIFY.\n")
        f.write(f"# If any change is needed, update {CONFIG_FNAME} instead.\n")
        f.write("set -u # all variables must be set\n")
        if not self.pred.pass_unmatched_args:
            cmd_str = \
                f'if [ ! $# = 0 ]; then\n' \
                f'    echo "ADW: Unknown arguments: $*"\n' \
                f'    exit 1\n' \
                f'fi\n'
            f.write(cmd_str)
        ...
        deps_dir = f"$ADW_PROJ_ROOT_DIR/{CMDS_DIR}/{target_dir}/{DNAME_DEPS}"
        if self.pred.is_exclusive:
            cmd_str = \
                f'if [ -d "{deps_dir}" ]; then\n' \
